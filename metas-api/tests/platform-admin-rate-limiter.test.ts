...
await test('Redis limiter hashes identity parts and shares the atomic counter', async () => {
  ...
  assert.doesNotMatch(keys[0] ?? '', /admin-sensitive|session-sensitive|203\.0\.113\.9/u);
});
