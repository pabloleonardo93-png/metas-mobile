...
export function EmployeeFormScreen({ mode }: EmployeeFormScreenProps) {
  ...
  const { addEmployee, employees, updateEmployee } = useEmployees();
  ...
  function handleSubmit(values: EmployeeFormValues & { role: UserRole }) {
    const input = {
      email: values.email,
      name: values.name,
      role: values.role,
    ...
    if (mode === 'edit' && employee) {
      ...
      Alert.alert('Funcionário atualizado', 'As alterações foram salvas localmente nesta sessão.');
      router.replace(appRoutes.managerEmployeeDetails(employee.id));
      return;
    }

    const newEmployee = addEmployee(input);
    Alert.alert('Funcionário adicionado', 'O cadastro foi salvo localmente para demonstração.');
